...
class MAML:
    ...
    def _outer_step(self, task_batch, train):  # pylint: disable=unused-argument
        ...
        for task in task_batch:
            ...
            params, acc_support = self._inner_loop(images_support, labels_support, True)
            pred_query = self._forward(images_query, params)
            loss = F.cross_entropy(pred_query, labels_query)
            acc_query = util.score(pred_query, labels_query)
            # # #
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_way', type=int, default=5, # 5
